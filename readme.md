...
- Express/Egg：基于 Node.js 的 Web 框架。Express 轻量灵活，Egg 是阿里出的企业级框架（约定优于配置）。
- 中间件 (Middleware)：像洋葱模型，请求进来经过一层层处理（日志、鉴权、解析），响应出去再经过一层层。
- 进程守护 (PM2)：Node.js 单线程挂了怎么办？用 PM2 集群模式，挂了一个自动重启，还能利用多核 CPU。
- MongoDB：NoSQL 数据库，存 JSON 格式数据，适合博客、日志这种结构不固定的场景。
- JWT (JSON Web Token)：无状态登录方案，比 Session 更适合前后端分离。
...
## 全称
**BFF = Backend for Frontend**  
翻译成人话："前端专属的后端代理" 或者 "前端的小秘书"。

## 为什么要搞个 BFF？（没有它行不行？）

### 场景：没有 BFF 时（痛苦的前端）
假设你要开发一个"用户中心"页面，需要展示：
- 用户基本信息（调用 User-Service）
- 订单列表（调用 Order-Service）
- 消息通知（调用 Message-Service）
- 积分余额（调用 Point-Service）

前端的困境：
- **请求太多**：页面一加载，要发 4 次 HTTP 请求。网速慢的话，界面会闪一下，体验差。
- **数据拼接麻烦**：你要在 JS 里等这 4 个请求都回来（Promise.all），然后把数据拼在一起，再渲染。
- **字段不对应**：后端返回的字段可能是 `user_name`，你界面要显示 `userName`，还得转换。
- **跨域问题**：4 个服务可能域名都不一样，你要配 4 次跨域。

### 有了 BFF 后（爽翻的前端）
你在前端和那 4 个后端服务中间，加了一层 Node.js (BFF)。流程变成：
- 前端只发 1 次请求：`GET /api/user-center`
- BFF 层收到请求，它去后台帮你跑断腿：
  - 调 User 服务
  - 调 Order 服务
  - 调 Message 服务
  - 调 Point 服务
- BFF 把数据拼好，整理成你需要的格式（userName），返回给前端。
- 前端拿到最终数据，直接渲染。

## 核心价值（面试必背）
- **对前端**：请求次数从 N 次变 1 次，数据格式直接用，不用处理跨域。
- **对后端**：他们不用改接口来适应你，他们继续按微服务规范写就行。
- **谁来做**：通常由前端团队维护（因为最懂页面需要什么数据），这就是为什么前端要学 Node.js 的原因。

一句话总结 BFF：  
**"BFF 就是前端自己掌控的一个中间层，用来聚合数据、裁剪字段、适配接口，让前端开发更爽，性能更好。"**
...
## 场景模拟
想象一个旅客（请求 Request）要坐上飞机（业务逻辑 Controller）。  
他必须经过一道道关卡（中间件 Middleware）。流程是这样的（进去）：
...
3. **第三层：海关（参数校验中间件）**  
   检查你带的液体超没超标（参数合不合法）。  
   超标？扣下（返回 400 错误）。  
   没问题？放行 ➡️ 核心业务（Controller）。

**核心业务（Controller）**  
你终于坐上飞机了（执行具体逻辑，查数据库，返回结果）。

流程是这样的（出来）：  
飞机落地了，旅客要离开（响应 Response），他又得原路返回，再经过一层层关卡：
- 海关：看看你有没有带违禁品出去（数据脱敏，比如把手机号中间四位变成 \*\*\*\*）。
- 安检员：盖个章表示已登机（添加响应头）。
- 保安：记下来："张三走了"。（记录响应时间）
...
async function middleware1(ctx, next) {
  console.log('1. 进来前：记录日志');
  await next(); // 放行去下一层
  console.log('1. 出来后：记录耗时');
}

async function middleware2(ctx, next) {
  console.log('2. 进来前：检查 Token');
  await next(); // 放行去下一层
  console.log('2. 出来后：添加统一响应头');
}

async function controller(ctx) {
  console.log('3. 核心业务：查数据库');
  ctx.body = '成功';
}
```

## 执行顺序
1 进 -> 2 进 -> 3 核心 -> 2 出 -> 1 出

## 面试怎么吹？
"我理解中间件就是一个责任链模式。  
每个中间件只负责一件事（日志、鉴权、解析 Body），通过 `await next()` 把请求传递给下一个。  
这种洋葱模型的好处是解耦：  
我想加个'限流'功能，只需要插一个新的中间件，完全不用改核心业务代码。  
...
### 场景 1：聊项目架构
"当时我们那个 i 迪汇项目，后端是微服务架构，接口特别散。  
如果前端直接调，得发十几个请求，页面加载太慢。  
所以我提议引入 Node.js 做 BFF 层。  
前端只调 BFF 的一个接口，BFF 在内部并发调用所有微服务，聚合数据后返回。  
这样首屏加载时间直接从 2 秒优化到了 0.5 秒。"  
*(亮点：性能优化 + 架构设计)*

### 场景 2：聊代码规范
"我们在做权限管理时，借鉴了 Express 的洋葱模型思想。  
封装了一个全局的请求拦截器（类似中间件）。  
每个请求进来先过一遍'鉴权层'，再过一遍'日志层'，最后才到业务逻辑。  
这样就算以后要加'黑白名单'或者'限流'，只需要加个插件就行，不用改动业务代码。"  
*(亮点：设计模式 + 可维护性)*
